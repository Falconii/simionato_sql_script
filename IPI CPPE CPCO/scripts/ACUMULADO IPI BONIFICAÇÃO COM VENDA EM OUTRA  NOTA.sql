/* ACUMULADO IPI BONIFICA플O COM VENDA EM OUTRA NOTA
 
   SEPARA POR MESMO PRODUTO OU OUTRO PRODUTO

*/

DROP TABLE acumulado_cnpj_data_ipi_cppe_cpco_fora_nota;
GO

CREATE TABLE public.acumulado_cnpj_data_ipi_cppe_cpco_fora_nota  ( 
...
INSERT INTO 
      acumulado_cnpj_data_ipi_cppe_cpco_fora_nota(
        cnpj, 
        ano, 
        mes, 
        sistema,
    ...
    ,CASE
     ...
     END AS PRODUTO
    ,CASE
        WHEN (CAB.SISTEMA = 'SAP' OR CAB.SISTEMA = 'SAP2') AND CFOP.TAG = '03' THEN 'REMESSA DISTRIBUI플O DE BRINDES'
        WHEN (CAB.SISTEMA = 'SAP' OR CAB.SISTEMA = 'SAP2') AND CFOP.TAG = '02' THEN 'REMESSA EM DOA플O'  
...
SELECT dp.empresa,dp.unid,acu.cnpj,acu.ano,acu.mes,acu.sistema,acu.produto,acu.origem,acu.classificacao,acu.vlr_ipi,acu.vlr_ipi_corrigido
  FROM   acumulado_cnpj_data_ipi_cppe_cpco_fora_nota ACU
  LEFT   JOIN depara dp on dp.cnpj = acu.cnpj
  ORDER BY dp.empresa,dp.unid,acu.cnpj,acu.ano,acu.mes,acu.sistema,acu.origem
  GO
