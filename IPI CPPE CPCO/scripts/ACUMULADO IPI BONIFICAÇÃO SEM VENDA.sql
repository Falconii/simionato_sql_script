/* ACUMULADO IPI BONIFICAÇÃO SEM VENDA
 
   SEPARA POR MESMO PRODUTO OU OUTRO PRODUTO

*/

DROP TABLE acumulado_cnpj_data_ipi_cppe_cpco_sem_venda;
GO

CREATE TABLE public.acumulado_cnpj_data_ipi_cppe_cpco_sem_venda  ( 
...
INSERT INTO 
      acumulado_cnpj_data_ipi_cppe_cpco_sem_venda(
        cnpj, 
        ano, 
        mes, 
        sistema,
    ...
    WHERE VAL.VLR_IPI > 0 AND 
        (VE.DESCRICAO = 'SEM VENDA NESTA NOTA') 
        AND 
        (
        ( val.id = 4     ) OR 
        ( val.id = 9     ) OR 
...
SELECT dp.empresa,dp.unid,acu.cnpj,acu.ano,acu.mes,acu.sistema,acu.produto,acu.origem,acu.classificacao,acu.vlr_ipi,acu.vlr_ipi_corrigido
  FROM   acumulado_cnpj_data_ipi_cppe_cpco_sem_venda ACU
