CREATE OR REPLACE FUNCTION public.seekVenda_cppe_cpco(in _id int4, in _empresa text, in _local text, in _dtnf date, in _nro text, in _serie text,in _material text,in _nr_item text, 
out __empresa text, out __local text, out __dtnf date, out __nro text, out __serie  text, out __nr_item  text, out __material text, out __descricao text, out __qtd numeric(15,4) , out __vlr_contabil numeric(15,4),   out __bas_ipi numeric(15,2) , out __per_ipi numeric(6,2), out __vlr_ipi numeric(15,2) ,  out  __ct int4, out __sem_venda text, out __id_v int4 , out __nro_linha_v int4)
...
BEGIN
 ...
 __nro_linha_v := 0;
  FOR tempo in  SELECT det.*  
      ...
      INNER JOIN  public.nfe_det_comp comp on comp.id = det.id and comp.nro_linha = det.nro_linha
      WHERE det.id = _id and
            ...
            (DET.CFOP = '7551') OR 
            (DET.CFOP = '7651') OR 
            (DET.CFOP = '7654')) AND COMP.BEBIDA = 'S'
      LOOP      
