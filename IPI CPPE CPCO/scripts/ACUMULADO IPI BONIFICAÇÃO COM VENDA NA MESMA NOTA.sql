/* ACUMULADO IPI BONIFICAÇÃO COM VENDA NA MESMA NOTA
 
   SEPARA POR MESMO PRODUTO OU OUTRO PRODUTO

*/

DROP TABLE acumulado_cnpj_data_ipi_cppe_cpco_na_nota;
GO

CREATE TABLE public.acumulado_cnpj_data_ipi_cppe_cpco_na_nota  ( 
...
INSERT INTO 
      acumulado_cnpj_data_ipi_cppe_cpco_na_nota(
        cnpj, 
        ano, 
        mes, 
        sistema,
    ...
    ,CASE
        WHEN VE.MATERIAL = ''            THEN 'MESMO PRODUTO'
        WHEN VE.MATERIAL = DET.MATERIAL  THEN 'OUTRO PRODUTO'
        ELSE                                   'NÃO DEFINIDO '
    END AS PRODUTO
    ,CASE
    ...
    WHERE VAL.VLR_IPI > 0 AND 
        NOT((VE.DESCRICAO = 'SEM VENDA NESTA NOTA') OR (SUBSTR(VE.DESCRICAO,1,1) = '*'))
        AND 
        (
        ( val.id = 4     ) OR 
        ( val.id = 9     ) OR 
...
SELECT dp.empresa,dp.unid,acu.cnpj,acu.ano,acu.mes,acu.sistema,acu.produto,acu.origem,acu.classificacao,acu.vlr_ipi,acu.vlr_ipi_corrigido
  FROM   acumulado_cnpj_data_ipi_cppe_cpco_na_nota ACU
