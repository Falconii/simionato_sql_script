CREATE OR REPLACE FUNCTION "public"."poenotavendames_cppe_cpco" (in _id int4 , out _saida text) 
RETURNS text 
AS
$$
DECLARE
...
BEGIN
  ...
  FOR tempo in  SELECT DET.*
      ...
      INNER JOIN NFE_DET_NOTA_VENDA VE   ON VE.ID   = DET.ID AND VE.NRO_LINHA   = DET.NRO_LINHA AND sem_venda = 'S'
      WHERE DET.ID = _id 
      ...
      LOOP      
        ...
        from seekvendasembonificacaoV2(tempo.id,tempo.dtnf,tempo.nro,tempo.serie,tempo.material,_Origem)
